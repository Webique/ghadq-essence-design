...
const SplashScreen = ({ onComplete }: SplashScreenProps) => {
  ...
  return (
    <div 
      className={`fixed inset-0 z-[9999] bg-[#D7C4A0] flex items-center justify-center transition-all duration-1000 ease-in-out ${
    ...
    >
      <div className="text-center">
        ...
        <div className={`transition-all duration-1000 ease-out ${textVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-8'}`}>
          <h1 className="text-4xl md:text-6xl lg:text-8xl font-bold text-[#846745] mb-4 font-arabic">
            غدق الإبداع
          </h1>
          <h2 className="text-2xl md:text-4xl lg:text-6xl font-bold text-[#9A8365] font-poppins">
            Ghadq AlEbda
          </h2>
        </div>
        ...
        <div className={`mt-8 transition-all duration-1000 ease-out ${logoVisible ? 'opacity-100 scale-100' : 'opacity-0 scale-75'}`}>
          <div className="w-16 h-16 md:w-20 md:h-20 lg:w-24 lg:h-24 mx-auto bg-[#AE8F65] rounded-full flex items-center justify-center">
            <div className="w-8 h-8 md:w-10 md:h-10 lg:w-12 lg:h-12 bg-[#846745] rounded-full"></div>
          </div>
        </div>

        {/* Loading Bar */}
        <div className="mt-12 w-48 md:w-64 mx-auto">
          <div className="w-full bg-[#C1AB85] rounded-full h-2">
            <div 
              className="bg-[#846745] h-2 rounded-full transition-all duration-3000 ease-out"
        ...
        <div className={`mt-8 transition-all duration-1000 ease-out delay-500 ${logoVisible ? 'opacity-100' : 'opacity-0'}`}>
          <p className="text-[#9A8365] text-lg md:text-xl font-medium">
            {language === 'ar' 
